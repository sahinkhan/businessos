...
External/manual `RecordAuditLogV2` commands remain a separate authorization path governed by `foundation.audit.write`. The internal appender is module-side mandatory evidence, not end-user impersonation authority. A person authorized to perform an audited business action does not need `foundation.audit.write` merely for the owning module to append required evidence.
...
| ADR-016 and ADR-019 | ADR-019 remains the accepted successor preserving Audit V2 semantics except the replaced worker-binding model. This binding supplies the missing Path A module proof. It does not replace `TenantExecutionBinding` or actual actor verification. |
