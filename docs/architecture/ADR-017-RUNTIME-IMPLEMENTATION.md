...
The Python 3.13 PostgreSQL proof covers fresh forward migration, dirty legacy preflight, overlap rejection, exact policy/action combinations, future and expired policies, half-open boundary predicates, tenant RLS, declared entity types, owner facts and capability checks, both hold/purge orders, release racing purge, duplicate purgers, policy replacement concurrent with purge, category/anchor mutation races, owner rollback, Audit rollback, committed cleanup outbox, and provider drain/replacement. Additional exact-head CI and independent read-only audit results belong to the implementation PR before owner attestation.
