...
Status: STANDALONE LOCAL CERTIFICATION PASS / HOSTED CERTIFICATION PENDING
...
| Full integration | 150 passed in 2236.92 seconds after the standalone authority test additions |
| Full pytest | 676 passed in 2873.34 seconds on the final complete run |
| Installed-wheel migration smoke | PASS; source and installed graphs match, role transition, upgrade, replay and safe downgrade refusal checked |
| Development, production, migration-smoke images | Final source rebuilds PASS after the role-preflight correction; migration plans PASS with required operator inventory |
| Repository mypy command | PASS; 238 source files after removing ignored wheel-build copies |
| Ruff and `git diff --check` | PASS after the focused certification changes |
...
The ADR-022 forward revisions now check Governance role attributes, membership, object ownership, and target-database connectivity immediately before each authority grant. A disposable fresh PostgreSQL volume with `businessos_governance BYPASSRLS` originally allowed `businessos migrate upgrade heads` to exit 0. After the forward-only correction, the same probe exited 1 with `unsafe Governance migration role prerequisites`; the migration transaction left no Governance, Audit, outbox, or Alembic schema behind. Both disposable volumes were removed. Hosted exact-head CI and an independent read-only audit are pending. No owner attestation has been posted.
