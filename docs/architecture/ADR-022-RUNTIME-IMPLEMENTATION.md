...
Every protected command admission validates role attributes, membership, object ownership, effective table and column grants, indirect ordinary write surfaces, and forced tenant policies before leasing the protected pool. This remains fail closed when readiness probes are disabled or have not run. Rotation performs the same inspection and is pinned to the configured database endpoint; a failed endpoint or privilege check disables new protected admissions. A generation entering drain is rejected before a new protected pool lease after an authorization wait. The ordinary and protected pools have bounded overflow, and startup checks the declared installation connection budget against PostgreSQL capacity; the declared budget includes configured app processes and worker, operations, and other connection reservations.
...
The first hosted candidate `1e7165d2431eba8523e376cfc459fe2425364e23` passed exact-head CI run `36250890219`, but its independent read-only audit failed at Critical 0, High 2, Medium 1, Low 0. Its remediation candidate `fa6a5e4f7c691409b04b47f9f86470c67c058e63` passed the local gates; hosted run `36259101359` found one nonportable wrong-endpoint test, and the next independent audit found Critical 0, High 1, Medium 2, Low 0: protected admission did not itself inspect grants, rotation missed column and indirect grants, and installation pool demand lacked a budget check. The subsequent correction validates authority at protected admission, inventories effective column and indirect writes, bounds connection demand, and constructs the wrong-endpoint test URL independently of the Docker hostname. Focused ADR-022 authority tests passed 8/8, conformance passed 18/18, and adjacent Audit/Eventing/migration/dispatcher tests passed 31/31. A dedicated integration run passed 155 tests in 40:07 before the final startup-budget condition was narrowed for conformance-only startup; the full pytest suite then passed 682 tests in 50:45 on the final runtime code, including all integration tests. Installed-wheel migration smoke, all three image builds, and a disposable installed-image transition/upgrade/replay passed. Exact-head hosted CI and a fresh independent read-only audit remain required before owner attestation. No owner attestation has been posted.
