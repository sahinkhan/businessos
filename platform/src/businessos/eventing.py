...
type PublisherAdmission = Callable[[SQLAlchemyUnitOfWork], AbstractAsyncContextManager[None]]
...
async def _subscriber_scope(
    ...
    if admission is None:
        ...
        async with admission(unit_of_work, transaction, subscriber) as binding:
            yield binding
...
class OutboxPublisher:
    ...
    async def _publish_locked_batch(self, unit_of_work: SQLAlchemyUnitOfWork, limit: int) -> int:
        published = 0
...
class DurableEventConsumer:
    ...
    async def consume(
        ...
        admission: SubscriberAdmission | None = None,
    ) -> int:
                        ...
                        async with unit_of_work:
                            ...
                            async with _subscriber_scope(
                                ...
                                await self._events.invoke_registered(
                                    ...
                                    event,
                                    EventHandlingContext(
                                        traced_context, dependencies, transaction, binding
                                    ),
                                )
                                await unit_of_work.commit()
                                processed += 1
