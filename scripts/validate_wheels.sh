...
"$environment_dir/bin/python" -c \
    "from importlib.resources import files; from pathlib import Path; root=Path('$repository_root').resolve(); packages=('businessos','businessos_tenant','businessos_identity','businessos_organization','businessos_geography','businessos_reference_data','businessos_uom','businessos_party','businessos_policy','businessos_audit','businessos_data_governance','businessos_proof'); roots=[Path(str(files(name))).resolve() for name in packages]; assert all(root not in item.parents for item in roots); assert roots[0].joinpath('migration_assets','env.py').is_file(); assert roots[1].joinpath('migrations','versions','tenant_0001_foundation.py').is_file(); assert roots[2].joinpath('migrations','versions','identity_0001_foundation.py').is_file(); assert roots[2].joinpath('migrations','versions','identity_0002_device_principal_integrity.py').is_file(); assert roots[3].joinpath('migrations','versions','organization_0001_foundation.py').is_file(); assert roots[3].joinpath('migrations','versions','organization_0002_typed_principals.py').is_file(); assert roots[4].joinpath('migrations','versions','geography_0001_foundation.py').is_file(); assert roots[5].joinpath('migrations','versions','reference_0001_foundation.py').is_file(); assert roots[6].joinpath('migrations','versions','uom_0001_foundation.py').is_file(); assert roots[7].joinpath('migrations','versions','party_0001_foundation.py').is_file(); assert roots[8].joinpath('migrations','versions','policy_0001_foundation.py').is_file(); assert roots[8].joinpath('migrations','versions','policy_0002_certification.py').is_file(); assert roots[9].joinpath('migrations','versions','audit_0001_foundation.py').is_file(); assert roots[9].joinpath('migrations','versions','audit_0002_immutability.py').is_file(); assert roots[10].joinpath('migrations','versions','gov_0001_foundation.py').is_file(); assert roots[10].joinpath('migrations','versions','gov_0002_runtime_access.py').is_file(); assert roots[11].joinpath('migrations','versions','0003_proof_atomic_state.py').is_file()"
